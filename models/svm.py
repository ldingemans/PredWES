...
def run_svm(X_train_val, y_train_val, X_real_test, y_real_test, MAX_EVALS, OUTER_FOLD_COUNT):
    """
    ...
    space4svm['X_train_val'] = X_train_val
    space4svm['y_train_val'] = y_train_val
    
    trials = Trials()
    best = fmin(f, space4svm, algo=tpe.suggest, max_evals=MAX_EVALS, trials=trials,early_stop_fn=no_progress_loss_moving_average(iteration_stop_count=20, percent_increase=0.0))
    ...
    for k in best:
       best[k] = best[k][0]
    best['kernel'] = ['linear', 'sigmoid', 'poly', 'rbf'][best['kernel']]
    ...
    for train_index_inner, test_index_inner in skf_inner.split(X_train_val, y_train_val):
            ...
            results = fill_results_array(results, 'SVM', str(OUTER_FOLD_COUNT) + '_' + str(current_inner_fold), train_means, val_means, result_indexer, False, y_train, y_val)
            brier_scores = fill_results_array(brier_scores, 'SVM', str(OUTER_FOLD_COUNT) + '_' + str(current_inner_fold), train_means, val_means, result_indexer, True, y_train,y_val)
    ...
    results = fill_results_array(results, 'SVM', str(OUTER_FOLD_COUNT) + '_' + str(current_inner_fold), train_means, val_means, result_indexer, False, y_train_val, y_real_test)
    brier_scores = fill_results_array(brier_scores, 'SVM', str(OUTER_FOLD_COUNT) + '_' + str(current_inner_fold), train_means, val_means, result_indexer, True, y_train_val,y_real_test)
